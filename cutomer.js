...
amqp.connect('amqp://localhost')
  .then(conn=> {
    return conn.createChannel().then(ch => {
        // Deklarasi antrian
      const queue1 = ch.assertQueue('Test', { durable: false })
      if(queue1){
        queue1.then(() => {
          /* Menangkap pesan yang dikirimkan oleh RabbitMQ */
          return ch.consume('Test', msg => console.log('- Received', msg.content.toString()), { noAck: true })
        })
        .then(() => {
      ...
      if(queue2){
        queue2.then(() => {
          ...
          return ch.consume('queue2', msg => {
            ...
            console.log(`His title is ${json_data.title}`);
          }, { noAck: true })
        })
        .then(() => {
          console.log('* Waiting for messages. Ctrl+C to exit')
